...
class CausalAttention(nn.Module):
    ...
    def __init__(self, config: ModelConfig):
        ...
        # d_model:隐藏维度,token向量长度; n_head:注意力头数 
        ...
        # self.d_attention * config.n_heads = config.d_model
        ...
        # Causal mask
        # 维度: 1, 1, context_length, context_length
    ...
    def forward(
        self, x: Float[Tensor, "batch seq_len d_model"]# 形状注释
        ...
        #[B, T, H, D]
        #[B, H, T, D]
        q = q.view(batch, seq_len, self.n_heads, self.d_attention).transpose(1,2)
        k = k.view(batch, seq_len, self.n_heads, self.d_attention).transpose(1,2)
        v = v.view(batch, seq_len, self.n_heads, self.d_attention).transpose(1,2)
        # [B, H, T, D]
        attention_score = q @ k.transpose(-2, -1)
        attention_score = attention_score/math.sqrt(self.d_attention)
        mask = self.causal_mask[:, :, :seq_len, :seq_len]
        attention_score = attention_score.masked_fill(mask == 0, float('-inf'))
        # [B, H, T, T]
        attention_out = softmax(attention_score, dim = -1)
        # [B, H, T, D]
        out = attention_out @ v
        # [B, T, H, D]
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        # TODO complete
        out = x + self.attention(self.pre_layer_norm(x))
        out = out + self.mlp(self.post_layer_norm(out))
        return out
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        x = self.embeddings(x) + self.position_embeddings(position)
