...
class GPT2Layer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    ### YOUR CODE HERE
    out = self.attention_layer_norm(hidden_states)
    out = self.self_attention(out, attention_mask)
    mid = self.add(hidden_states, out, self.attention_dense, self.attention_dropout)

    out1 = self.out_layer_norm(mid)
    out1 = self.interm_dense(out1)
    out1 = self.interm_af(out1)
    out1 = self.add(mid, out1, self.out_dense, self.out_dropout)
    return out1
