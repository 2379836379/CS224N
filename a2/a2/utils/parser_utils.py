...
class Parser(object):
    ...
    def create_instances(self, examples):
        max_instances = sum((len(ex['word']) - 1) * 2 for ex in examples)
        features = np.empty((max_instances, self.n_features), dtype=np.int32)
        labels = np.empty(max_instances, dtype=np.int64)
        cursor = 0
        succ = 0
        for id, ex in enumerate(examples):
            ...
            for i in range(n_words * 2):
                ...
                features[cursor] = self.extract_features(stack, buf, arcs, ex)
                labels[cursor] = gold_t
                cursor += 1
        ...
        return features[:cursor], labels[:cursor]
