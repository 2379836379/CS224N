...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ### 36个特征，每个特征对应一个词向量， flatten之后输入隐藏层
        input_dim = self.n_features * self.embed_size
        self.hidden_weight = nn.Parameter(torch.empty(input_dim, self.hidden_size))
        self.hidden_bias = nn.Parameter(torch.empty(self.hidden_size))
        nn.init.xavier_uniform_(self.hidden_weight)
        nn.init.uniform_(self.hidden_bias)
        self.dropout = nn.Dropout(p = dropout_prob)
        self.logits_weight = nn.Parameter(torch.empty(self.hidden_size, self.n_classes))
        self.logits_bias = nn.Parameter(torch.empty(self.n_classes))
        nn.init.xavier_uniform_(self.logits_weight)
        nn.init.uniform_(self.logits_bias)
    ...
    def forward(self, w):
        ...
        hidden = torch.matmul(x, self.hidden_weight) + self.hidden_bias
        hidden = torch.relu(hidden)
        hidden = self.dropout(hidden)
        logits = torch.matmul(hidden, self.logits_weight) + self.logits_bias
